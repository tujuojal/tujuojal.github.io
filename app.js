 ...
 * Elevation  : NLS Finland WCS korkeusmalli_2m (2 m LiDAR DEM)
 ...
 *
 * NLS requests go through a Cloudflare Worker proxy that appends the
 * API key server-side (stored as a Worker secret, not in this repo).
 */
...
document.addEventListener('click', e => {
  if (panelOpen && !panel.contains(e.target) && !btnPanel.contains(e.target)) {
    setPanelOpen(false);
  }
});
